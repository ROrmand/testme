...
2. Run `npx testme generate` to create questions in `.testme/session.json`.
3. Answer each question in chat; the agent writes `.testme/answers.json` and `.testme/judgments.json`.
4. Run `npx testme verify` — on PASS, `.testme/pass.json` is written and git operations are allowed.
5. After a successful push, `PROMPTS.md` resets automatically via hook.

Do not commit or push until `npx testme verify` passes.
